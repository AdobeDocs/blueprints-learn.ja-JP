...
| データソースと収集 | 同じ位置に仮定 | コンバージョンイベントをキャプチャするには、CTA宛先のWeb SDKまたはAnalyticsのタグ付けがアクティブである必要があります。 プロファイル属性のストリーミングまたはバッチ取り込みパイプラインは運用可能である必要があります。 | [Web SDKの概要](https://experienceleague.adobe.com/ja/docs/experience-platform/web-sdk/home)、[&#x200B; ソースの概要](https://experienceleague.adobe.com/ja/docs/experience-platform/sources/home) |
...
| --- | --- | --- | --- |
| 計算属性/派生属性作成 | 推奨 | 最後の購入日、生涯注文数、エンゲージメントスコアなどの計算属性により、オーディエンスの精度を向上させ、より詳細なメッセージのパーソナライズを可能にします。 | [計算属性の概要](https://experienceleague.adobe.com/ja/docs/experience-platform/profile/computed-attributes/overview) |
| データライフサイクル管理 | 推奨 | コンバージョンの追跡を促進するイベントデータセットについては、データ保持ポリシー（有効期限）を導入する必要があります。 同意スキーマフィールドは、チャネルレベルのオプトイン/オプトアウトの適用用に設定する必要があります。 | [高度なデータライフサイクル管理の概要](https://experienceleague.adobe.com/ja/docs/experience-platform/data-lifecycle/home)、[同意と環境設定のフィールドグループ &#x200B;](https://experienceleague.adobe.com/ja/docs/experience-platform/xdm/field-groups/profile/consents) |
...
- [APIを使用したトリガーキャンペーン](https://experienceleague.adobe.com/ja/docs/journey-optimizer/using/campaigns/api-triggered-campaigns/api-triggered-campaigns)
