...
| --- | --- | --- | --- |
| 管理とガバナンス | 所定の位置に想定 | web チャネル権限が設定されたAJO サンドボックス。[!DNL Web SDK] 実装チームに付与される実装権限およびデータストリームアクセス。 Web チャネルの設定、オーディエンス管理およびキャンペーンの実行を可能にする役割がプロビジョニングされたユーザー。 | [&#x200B; アクセス制御の概要 &#x200B;](https://experienceleague.adobe.com/en/docs/experience-platform/access-control/home) |
| データモデリングと準備 | 必須 | Web 行動シグナル（ページビュー数、クリック数、スクロール深度、リファラルデータ、UTM パラメーター）をキャプチャするエクスペリエンスイベントスキーマ。 リアルタイムの評価をサポートするには、スキーマに標準の web インタラクションフィールドグループを含め、エッジプロファイルに対して有効にする必要があります。 対応するデータセットを作成し、プロファイルを有効にする必要があります。 | [XDM システムの概要 &#x200B;](https://experienceleague.adobe.com/en/docs/experience-platform/xdm/home) |
...
| ID とプロファイル設定 | 必須 | ECID （[!DNL Experience Cloud ID]）が、匿名訪問者のプライマリ ID 名前空間として設定されている。 エッジで匿名のプロファイルデータを解決するには、Edge結合ポリシーに `isActiveOnEdge: true` を設定する必要があります。 サンドボックスごとにエッジでアクティブにできる結合ポリシーは 1 つだけです。 | [ID サービスの概要 &#x200B;](https://experienceleague.adobe.com/en/docs/experience-platform/identity/home) |
| オーディエンスの定義とセグメント化 | 必須 | セッション内の行動シグナルに基づいて定義された、Edgeで評価されたオーディエンスセグメント。 Edgeのセグメント化は、2 秒未満の評価待ち時間には必須です。 セグメントルールでは、エッジ適格なセグメントルール式のみを使用する必要があります（単純な属性チェックとセグメントメンバーシップ – 時系列クエリや複雑な集計はありません）。 | [Edgeのセグメント化 &#x200B;](https://experienceleague.adobe.com/en/docs/experience-platform/segmentation/methods/edge-segmentation) |
...
| --- | --- | --- | --- |
| 計算/派生属性の作成 | 該当なし | 集計する履歴プロファイルデータが最小限なので、匿名訪問者に対する値は限られています。 エッジプロファイルが、複数のセッションをまたいだ以前の匿名訪問からの有意義な行動データを蓄積する場合に適用される可能性があります。 | [&#x200B; 計算済み属性の概要 &#x200B;](https://experienceleague.adobe.com/en/docs/experience-platform/profile/computed-attributes/overview) |
| データ・ライフサイクル管理 | 推奨 | 偽名プロファイルの有効期限は、ストレージを管理し、プライバシー要件に準拠するために、匿名エッジプロファイルに対して設定する必要があります。 ECID 専用プロファイルは、14 ～ 365 日の間で有効期限が切れるように設定できます。 行動データ収集には、cookie 同意ポリシーを適用する必要があります。 | [Advanced Data Lifecycle Management の概要 &#x200B;](https://experienceleague.adobe.com/en/docs/experience-platform/data-lifecycle/home) |
| データ使用のラベル付けと適用 | 推奨 | 行動データのガバナンスラベルは、特にジオターゲティング（S2 に対応する地理的ラベル）とデバイスベースのパーソナライゼーションのコンプライアンスを確保します。 ラベルを使用すると、権限のないパーソナライゼーションコンテキストで制限付きの行動データが使用されるのを防ぐことができます。 | [&#x200B; データガバナンスの概要 &#x200B;](https://experienceleague.adobe.com/en/docs/experience-platform/data-governance/home) |
| 監視と監視 | 推奨 | [!DNL Edge Network] および [!DNL Web SDK] データフロー監視は、パーソナライゼーション配信の問題を検出するのに役立ちます。 データストリームの失敗、取り込みエラー、エッジ配信の異常値に関するアラートを設定します。 パーソナライゼーションの失敗によって訪問者のエクスペリエンスが低下する実稼動のデプロイメントでは重要です。 | [Observability Insights の概要 &#x200B;](https://experienceleague.adobe.com/en/docs/experience-platform/observability/home) |
| レポートと分析 | Included | Personalization パフォーマンスレポートは、ファンクションチェーンの一部です（フェーズ 5）。 CJAの匿名訪問者パーソナライゼーションの有効性を分析することで、funnelのネイティブレポートよりも詳細なAJO分析、コホート比較、コンバージョンの影響の測定を行うことができます。 | [CJAの概要 &#x200B;](https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-overview/cja-overview) |
...
- Edge セグメントは、単純な属性チェックとセグメントメンバーシップ（時系列クエリや複雑な集計なし）に制限されています。[Edge セグメント化の実施要件 &#x200B;](https://experienceleague.adobe.com/en/docs/experience-platform/segmentation/methods/edge-segmentation)
- サンドボックスごとにエッジでアクティブにできる結合ポリシーは 1 つだけです。[&#x200B; プロファイルガードレール &#x200B;](https://experienceleague.adobe.com/en/docs/experience-platform/profile/guardrails)
...
- コンテンツ実験あたり最大 10 個の処理バリアント — [&#x200B; コンテンツ実験の制限 &#x200B;](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/get-started/guardrails)
- サンドボックス（オプション C）あたり最大 10,000 件の承認されたパーソナライズされたオファー – [&#x200B; 意思決定管理ガードレール &#x200B;](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/get-started/guardrails)
- 1 つの決定につき最大 30 個のプレースメント （オプション C） — [Journey Optimizer ガードレール &#x200B;](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/get-started/guardrails)
