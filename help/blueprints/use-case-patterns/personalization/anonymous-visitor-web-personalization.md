...
![匿名オーディエンスのアクティブ化とパーソナライゼーションのための参照アーキテクチャ ](/help/blueprints/audience-activation/assets/anonymous_activation.svg)
...
| --- | --- | --- | --- |
| 管理とガバナンス | 同じ位置に仮定 | web チャネル権限が設定されたAJO サンドボックス。 実装チームに付与された[!DNL Web SDK]個の実装権限とデータストリームアクセス。 web チャネルの設定、オーディエンスの管理、キャンペーンの実行を可能にする役割がプロビジョニングされたユーザー。 | [ アクセス制御の概要](https://experienceleague.adobe.com/en/docs/experience-platform/access-control/home) |
| データモデリングと準備 | 必須 | web行動シグナル（ページビュー、クリック、スクロールの深さ、紹介データ、UTM パラメーター）をキャプチャするエクスペリエンスイベントスキーマ。 スキーマには、標準のweb インタラクションフィールドグループが含まれており、リアルタイム評価をサポートするためにエッジプロファイルが有効になっている必要があります。 対応するデータセットを作成し、プロファイルを有効にする必要があります。 | [XDM システムの概要](https://experienceleague.adobe.com/en/docs/experience-platform/xdm/home) |
...
| IDとプロファイル設定 | 必須 | ECID （[!DNL Experience Cloud ID]）は、匿名の訪問者のプライマリ ID名前空間として設定されています。 エッジで匿名プロファイルデータを解決するには、Edge結合ポリシーを`isActiveOnEdge: true`で設定する必要があります。 サンドボックスごとにエッジでアクティブにできる結合ポリシーは1つだけです。 | [ID サービスの概要](https://experienceleague.adobe.com/en/docs/experience-platform/identity/home) |
...
| --- | --- | --- | --- |
| 計算属性/派生属性作成 | 影響しない | 匿名ユーザー向けの価値は限定的です。集約する過去のプロファイルデータは最小限に抑えるからです。 エッジプロファイルが、複数のセッションにわたる以前の匿名の訪問から有意義な行動データを収集する場合に適用される可能性があります。 | [計算属性の概要](https://experienceleague.adobe.com/en/docs/experience-platform/profile/computed-attributes/overview) |
| データライフサイクル管理 | 推奨 | ストレージを管理し、プライバシー要件に準拠するために、匿名のエッジプロファイルに仮名プロファイルの有効期限を設定する必要があります。 ECIDのみのプロファイルは、14日から365日の間で有効期限を設定できます。 行動データの収集には、Cookieの同意ポリシーを適用する必要があります。 | [高度なデータライフサイクル管理の概要](https://experienceleague.adobe.com/en/docs/experience-platform/data-lifecycle/home) |
| データ使用のラベル付けと適用 | 推奨 | 行動データにガバナンスラベルを付けることで、特に地域ターゲティング（S2機密ラベル）やデバイスベースのパーソナライゼーションの場合、コンプライアンスを確保することができます。 ラベルは、制限された行動データが、権限のないパーソナライゼーションコンテキストで使用されるのを防ぎます。 | [ データガバナンスの概要](https://experienceleague.adobe.com/en/docs/experience-platform/data-governance/home) |
| 監視と可観測性 | 推奨 | [!DNL Edge Network]と[!DNL Web SDK] データ フローの監視は、パーソナライゼーション配信の問題を検出するのに役立ちます。 データストリームの失敗、取り込みエラー、エッジ配信の異常値に対するアラートを設定します。 パーソナライゼーションの失敗によって訪問者のエクスペリエンスが低下する本番環境への導入で重要です。 | [ オブザーバビリティ インサイトの概要](https://experienceleague.adobe.com/en/docs/experience-platform/observability/home) |
| レポートと分析 | 含まれる | Personalizationのパフォーマンスレポートは、実行計画（フェーズ 5）の一部です。 匿名の訪問者のパーソナライゼーションの効果に関するCJA分析を使用すると、AJO ネイティブレポートが提供する内容を超えた、funnelの詳細な分析、コホート比較、コンバージョンの影響測定が可能になります。 | [CJAの概要](https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-overview/cja-overview) |
...
- Edge セグメントは、単純な属性チェックとセグメントメンバーシップに限定されます。時系列クエリや複雑な集計は使用できません。[Edge セグメントの適格性](https://experienceleague.adobe.com/en/docs/experience-platform/segmentation/methods/edge-segmentation)
- サンドボックスごとにエッジでアクティブにできる結合ポリシーは1つだけです – [ プロファイルガードレール ](https://experienceleague.adobe.com/en/docs/experience-platform/profile/guardrails)
...
- コンテンツ実験1回あたり最大10個の処理バリエーション — [ コンテンツ実験の制限](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/get-started/guardrails)
- サンドボックスごとに最大10,000件の承認済みパーソナライズされたオファー（オプション C） — [意思決定管理ガードレール ](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/get-started/guardrails)
- 1決定あたりの最大プレースメント数30件（オプション C） — [Journey Optimizer ガードレール ](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/get-started/guardrails)
