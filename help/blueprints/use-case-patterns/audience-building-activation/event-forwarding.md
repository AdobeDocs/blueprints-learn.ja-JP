...
| --- | --- | --- | --- |
| 管理とガバナンス | 必須 | サンドボックスは、適切なユーザーの役割と権限が設定された状態でアクティブである必要があります。 イベント転送を管理するユーザーには、イベント転送プロパティ、拡張機能、ルールを管理する権限など、[!DNL Adobe Admin Console]のデータ収集権限が必要です。 | [&#x200B; アクセス制御の概要](https://experienceleague.adobe.com/ja/docs/experience-platform/access-control/home) |
| データモデリングと準備 | 必須 | XDM スキーマは、Edge Networkを流れるイベントデータに対して定義する必要があります。 イベント転送ルールがフィルタリング、変換、マッピング用の構造化フィールドにアクセスできるように、データストリームは有効なXDM ExperienceEvent スキーマを参照する必要があります。 | [XDM システムの概要](https://experienceleague.adobe.com/ja/docs/experience-platform/xdm/home) |
| データソースと収集 | 必須 | Web SDK、モバイルSDK、Edge Network Server APIなどのデータ収集メカニズムは、設定されたデータストリームを通じてイベントを送信するために、アクティブである必要があります。 データストリームは、クライアントサイドの収集をサーバーサイドのイベント転送に接続する基本的なルーティング層です。 | [&#x200B; データストリームの設定](https://experienceleague.adobe.com/ja/docs/experience-platform/datastreams/configure) |
...
| オーディエンスの定義とセグメント化 | 影響しない | イベント転送では、個々のイベントをリアルタイムで処理し、オーディエンスメンバーシップは評価されません。 オーディエンスベースのフィルタリングは、イベント転送実行計画の一部ではありません。 オーディエンスベースのアクティベーションが必要な場合は、Audience Activation宛先リファレンスプランを参照してください。 | |
...
| データライフサイクル管理 | 推奨 | イベントデータを（同じデータストリームを介して）AEP データセットにも取り込む場合は、それらのデータセットに対してデータリテンションポリシー（有効期限）を設定して、ストレージコストと規制遵守を管理する必要があります。 イベント転送自体はデータを保存しませんが、並行AEP取り込みパスは保存します。 | [高度なデータライフサイクル管理の概要](https://experienceleague.adobe.com/ja/docs/experience-platform/data-lifecycle/home) |
| データ使用のラベル付けと適用 | 推奨 | イベント転送ルールは、フィールドレベルのフィルタリングを提供しますが（転送されたペイロードから機密データを除外することができます）、データ使用ラベルを基礎となるスキーマとデータセットに適用することで、同じデータがオーディエンスのアクティベーションやパーソナライゼーションに使用される場合、ガバナンスポリシーが適用されます。 | [&#x200B; データガバナンスの概要](https://experienceleague.adobe.com/ja/docs/experience-platform/data-governance/home) |
| 監視と可観測性 | 含まれる | イベント転送には監視が不可欠です。 イベント転送監視ダッシュボードでは、転送成功率、エラー率、宛先応答コードを可視化できます。 宛先のエラーに対してアラートを設定する必要があります。 | [&#x200B; イベント転送モニタリング &#x200B;](https://experienceleague.adobe.com/ja/docs/experience-platform/tags/event-forwarding/monitoring) |
| レポートと分析 | 推奨 | 転送されたイベントがサードパーティの分析プラットフォームにフィードする場合は、統一されたクロスチャネルビューのために、同じAEP イベントデータセットをCJAに接続することを検討してください。 これにより、Adobe側とサードパーティ側の分析を比較することができます。 | [CJAの概要](https://experienceleague.adobe.com/ja/docs/analytics-platform/using/cja-overview/cja-overview) |
...
- 拡張機能の使用状況は異なります。計画する前に、[&#x200B; データ コレクション拡張機能カタログ &#x200B;](https://experienceleague.adobe.com/ja/docs/experience-platform/tags/extensions/server/overview)を確認してください
...
**アプリケーション機能：** AEP: イベントルールの定義、AEP：宛先マッピング
