...
| --- | --- | --- | --- |
| 計算属性/派生属性作成 | 推奨 | 計算属性を使用することで、個人レベルのエンゲージメントイベント（メール開封、コンテンツのダウンロード、ウェビナーへの参加）をアカウントレベルのエンゲージメント指標に集約し、購買グループのスコアリングやアカウントの適格性ロジックに活用することができます。 | [計算属性の概要](https://experienceleague.adobe.com/en/docs/experience-platform/profile/computed-attributes/overview) |
| データライフサイクル管理 | 推奨 | 同意管理は、B2B メールとSMS コミュニケーションにとって不可欠です。 データセットの有効期限ポリシーは、一時的なエンゲージメントデータのライフサイクルを管理し、データ保持要件へのコンプライアンスを確保するのに役立ちます。 | [高度なデータライフサイクル管理](https://experienceleague.adobe.com/en/docs/experience-platform/data-lifecycle/home) |
| データ使用のラベル付けと適用 | 推奨 | B2B データには、多くの場合、企業の機密情報や取引先責任者の個人データが含まれます。 データガバナンスポリシーにより、特に広告プラットフォームやサードパーティシステムに対してアクティベートする際に、宛先をまたいでB2B データをコンプライアンスを維持して使用できます。 | [ データガバナンスの概要](https://experienceleague.adobe.com/en/docs/experience-platform/data-governance/home) |
| 監視と可観測性 | 推奨 | 監視により、B2B データパイプライン（CRM/[!DNL Marketo]同期）が正常に動作し、アカウントプロファイルが更新され、アカウントジャーニーの実行が失敗することなく続行されます。 ソースデータフローのエラーに関するアラートは、データの通貨を維持するために重要です。 | [ オブザーバビリティ インサイトの概要](https://experienceleague.adobe.com/en/docs/experience-platform/observability/home) |
| レポートと分析 | 含まれる | [!DNL AJO B2B Edition]内のB2B分析ダッシュボードは、購買グループのエンゲージメント、アカウントジャーニーのパフォーマンス、パイプラインの指標を提供します。 [!DNL CJA B2B Edition]は、アカウントレベルのワークスペース分析、購買グループ分析、および商談相関関係を使用して分析を拡張します。 | [CJAの概要](https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-overview/cja-overview) |
...
- [購買グループの作成](https://experienceleague.adobe.com/en/docs/journey-optimizer-b2b/user/buying-groups/buying-groups-overview)
...
- [アカウント選定](https://experienceleague.adobe.com/en/docs/journey-optimizer-b2b/user/buying-groups/buying-group-stages)
- [AJO B2BのAI アシスタント](https://experienceleague.adobe.com/en/docs/journey-optimizer-b2b/user/guide-overview)
...
**アプリケーション機能：** [!DNL RT-CDP B2B]: アカウントプロファイルの統合、B2B ID解決、[!DNL Marketo Engage]統合、B2B データガバナンス、アカウントオーディエンスの評価
...
**アプリケーション機能：** [!DNL AJO B2B]：ソリューションへの関心の設定、購買グループの管理
...
**アプリケーション機能：** [!DNL AJO B2B]: エンゲージメントスコアリング、アカウント選定
...
**アプリケーション機能：** [!DNL AJO B2B]: アカウント Journey Orchestration、B2B メールオーサリング、SMS Channel Management
...
**アプリケーション機能：** [!DNL AJO B2B]: Sales Alert Configuration、CRM Sales Insights; [!DNL RT-CDP B2B]: Account Destination Configuration、Account Audience Activation
...
**アプリケーション機能：** [!DNL AJO B2B]: B2B Analytics ダッシュボード

このフェーズでは、購買グループのパフォーマンス、アカウントジャーニーの有効性、パイプラインへの影響を測定するためのレポートと分析のフレームワークを確立します。 [!DNL AJO B2B Edition]には組み込みの分析ダッシュボードが用意されています。[!DNL CJA B2B Edition] （ライセンスされている場合）は、より詳細なクロスチャネルのアカウントレベルのインサイトを使用して分析を拡張します。
...
- [B2B分析ダッシュボード](https://experienceleague.adobe.com/en/docs/journey-optimizer-b2b/user/dashboards/buying-groups-dashboard)
...
- 同時最大ジャーニーと1 ジャーニーあたりの最大顧客数を含む[!DNL AJO B2B Edition]のアカウントジャーニー制限は、[!DNL AJO B2B Edition]製品ガードレール — [AJO B2B ガードレール ](https://experienceleague.adobe.com/en/docs/journey-optimizer-b2b/user/guide-overview)に従います
- [!DNL RT-CDP B2B Edition]は、最大50個のB2B スキーマクラスをサポートし、標準プロファイルとセグメント化ガードレールに従います。[ リアルタイム顧客プロファイルガードレール ](https://experienceleague.adobe.com/en/docs/experience-platform/profile/guardrails)
...
- B2B ソースコネクタの取り込みには、最小スケジュール間隔（通常、[!DNL Marketo]で15分、CRM ソースによって異なります）が含まれます – [取り込みガードレール ](https://experienceleague.adobe.com/en/docs/experience-platform/ingestion/guardrails)
- メールチャネルサーフェスは、サンドボックスごとに1つのチャネルタイプにつき10個に制限されます – [Journey Optimizer ガードレール ](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/get-started/guardrails)
