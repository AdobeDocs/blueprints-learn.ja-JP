...
description: Adobe [!DNL Experience Platform]  （AEM）を使用して、データレイク内のデータに対して探索的クエリと分析を実行します。
...
[!DNL Experience Platform] を使用すると、サードパーティの SQL クライアント、インターフェイス、Business Intelligence（BI）ツールとの接続が、[!DNL Experience Platform] プロトコルを使用して、[!DNL PostgreSQL] 内のデータに直接接続し、アクセスし、クエリを実行できます。
