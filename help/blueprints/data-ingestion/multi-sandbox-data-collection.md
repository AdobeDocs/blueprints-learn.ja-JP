...
1. クライアントで収集されたデータは、 [!DNL Edge Network]. また顧客は、サーバー側の収集方法として最初に独自のサーバーにデータを送信することもできます。Web SDK は、サーバー間収集機能を提供します。ただし、この実装には別のプログラミングモデルが必要です。ドキュメントを参照してください。 **[!DNL Edge Network]サーバー API の概要** below

1. Platform [!DNL Edge Network] データ収集ペイロードを受け取り、Target や Analytics などの必要なシステムへのデータのフローを調整します。
