...
アップグレードの対象となる顧客を、場所や使用パターンにもとづいてパーソナライズされたオファーやメリットでターゲティングできます。 新しいカバレッジが拡大するにつれ、新しくカバーした領域で関連するメッセージを加入者に配信することで、導入が加速し、ネットワーク稼働率が向上します。

### ビジネスへの影響

ターゲットを絞ったアップグレードキャンペーンにより、適格な購読者の採用率を向上させ、ネットワーク投資のリターンと競争力のある差別化をサポートします。

### 導入方法

[&#x200B; バッチアウトバウンドメッセージのアクティベーション &#x200B;](/help/blueprints/use-case-patterns/campaign-management-orchestration/batch-outbound-message-activation.md) パターンを使用して、カバー範囲の可用性、デバイスの互換性、プランの適格性に基づいて購読者をセグメント化し、各購読者の使用プロファイルに最も関連するメリットを強調する、パーソナライズされたアップグレードキャンペーンを配信します。 これは、オーディエンスが事前に定義され、大規模で、配信タイミングがイベント主導ではなくスケジュールされ、リアルタイムの分岐や決定が必要ない場合に適したパターンです。キャンペーンは、カバー範囲のタイムラインにもとづいて、事前に完全に計画することができます。

### 技術的な考慮事項

- ネットワークカバレッジマップを統合することで、アクティブなサービスが提供されている地域の加入者を正確に特定し、まだカバレッジできない地域でのアップグレードの促進を避けることができます。
- デバイスの互換性に関するデータを接続して、新しいデバイスが必要な加入者と、既に使用可能なハードウェアを持っている加入者とを判断します。
