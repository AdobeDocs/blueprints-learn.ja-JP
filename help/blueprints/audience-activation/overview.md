...
description: Real-time Customer Data Platformを使用して、アクティブ化されたオーディエンスおよびプロファイル中心のカスタマーエクスペリエンスを提供し​ます。
...
# オーディエンスとプロファイルアクティベーションのブループリント
...
* [!UICONTROL ザReal-time Customer Data Platform] は、オーディエンスのメンバーシップに加え、アクティベーション用に選択されたセグメントのメンバーであるプロファイルに対して発生する属性および ID の変更をアクティブ化できます。 属性やアイデンティティをアクティブ化することが目的の場合は、属性やアイデンティティのアップデートが送信されるすべてのプロファイルを含むグローバルセグメントを定義する必要があります。この時点で、セグメントと目的の属性を選択し、宛先設定の一部としてアクティブ化できます。
