...
# Audience CollaborationとSegment Match ブループリント

>[!TIP]
>このブループリントは、「オーディエンスの構築とアクティベーション」の下の[ ユースケースパターン ](/help/blueprints/use-case-patterns/audience-building-activation/audience-collaboration-segment-match.md)としても利用できます。
...
## プログラマティック契約でのセグメントマッチの使用

サードパーティ Cookieとデバイス識別子に対する制限が強化される中、プログラマティック広告は、オーディエンスを構築してターゲティングする新たな方法を模索しています。 「ユニバーサル ID」ソリューションの数は増えていますが、業界はまだ流動的であり、適用されるプライバシーに関する懸念のバランスを取りながら、同じレベルのターゲティングを達成するための合意されたスケーラブルな方法はありません。

Adobe Experience Platformのセグメントマッチは、プライバシー保護を重視したオーディエンスとのコラボレーションに活用し、広告主とパブリッシャー間のプログラマティックなプライベート契約を促進します。 セグメントマッチでは、次のことができます。

* **広告取引**&#x200B;と&#x200B;**オーディエンス**&#x200B;のワークフローを分割します。
* パートナー企業は、同意を得たプロセス内でハッシュ化された電子メールや電話番号などの永続的な識別子を使用して、相互に共有および同意するIDのオーディエンスメタデータを共有できます。

### ユースケース

* プログラマティックなプライベート取引を通じて、ファーストパーティオーディエンスをターゲットに。
* プログラマティックなプライベート取引による1st パーティオーディエンスの抑制。
* プログラマティックなプライベート契約を通じてシードされた、ファーストパーティオーディエンスの類似オーディエンスをターゲティングします。

>[!BEGINSHADEBOX]

**ブランド （Luma）とメディアネットワーク （ACME）の間の次のワークフロー例を考えてみましょう。**

1. ブランド（Luma）は、セグメントマッチを介してメディアネットワーク（ACME）とオーディエンスマッチを行います。
2. ACMEは、Adobe Real-Time CDP Destinationを介して、オーディエンスをアドサーバーまたはプログラマティック SSPにプッシュします。
3. ACMEは、前の手順で設定したオーディエンスを含む、該当するターゲティング基準を使用して、プライベート在庫取引（ID）を設定します。 その後、プライベート在庫取引IDがLumaのDSPにプッシュされます。
4. Lumaは、プライベート在庫取引とトラフィックキャンペーン/広告クリエイティブを設定します。
5. その後、このキャンペーンは、プログラマティックなプライベート在庫取引によって提供されます。
6. 次に、広告サーバーまたはSSPは、確立されたターゲティング基準を満たす広告インプレッションを配信します。 （保証取引または優先取引が契約で確立されたかどうかに応じて、広告サーバーやDSPを通じて、頻度の上限などの追加のターゲティング基準を利用できます）。
7. トラフィックはLumaのブランドプロパティに誘導されます。
8. そして、ACMEは、キャンペーン後のインサイトやオーディエンスを、セグメントマッチを介して共有し、再ターゲティングします。

>[!ENDSHADEBOX]

![ ブランドとパブリッシャー間のワークフローの図。](./assets/segment-match-blueprints.png)

>[!IMPORTANT]
>
> 上記のソリューションは、プログラマティックなプライベート取引を介してファーストパーティデータをターゲットにする簡単な方法を提供しますが、実行する前に次のような考慮事項がある場合があります。
>
>* 同意：この方法でデータを活用するために、ブランド、パブリッシャー、または小売メディアネットワークによる適用可能な同意収集。
>
>* ポリシーとライセンス契約：ブランド、パブリッシャー、または小売メディアネットワークが適用するポリシー（プライバシーポリシー、サードパーティベンダー契約を含む）に準拠し、この方法でデータを活用してアクティベートします。
