...
description: Adobe Experience Platform（AEP）の [!UICONTROL Segment Match] について説明します。[!UICONTROL Segment Match] は、共通の業界識別子に基づいて、安全で管理された、プライバシーに優しい方法でセグメントデータを交換できるデータコラボレーションサービスです。
...
# Segment Match ブループリントのオーディエンス Collaboration
...
「発行者のユースケース」は、サードパーティ Cookie とモバイル広告 ID データの廃止による最も大きな影響を受けます。 このユースケースは、ビジネスモデルとして広告の販売に重点を置くメディアおよびエンターテイメント業界に大きな影響を与えます。[!UICONTROL Segment Match] は、大規模なファーストパーティオーディエンスを持ち、広告主との直接共同作業を検討しているパブリッシャー向けのパスです。 広告主は発行者と直接連携して、詳細なターゲティングや予測キャンペーンのために、発行者のプロパティで、一致したオーディエンスに対して広告を出すことができます。
...
[!UICONTROL セグメントの一致]は、データを購入することができるデータマーケットプレイスではありません。 むしろ、一部のパートナーとファーストパーティデータを連携させるAEP機能であり、プライバシーと同意のコントロールを使用して共同作業を支援します。[!UICONTROL Segment Match] は、顧客関係の改善とブランドの成長に取り組みを集中させるのに役立ちます。 既存のブランドやパートナーとの関係が存在する場合に役立ちます。[!UICONTROL Segment Match] エクスペリエンスは管理が容易で拡張性が高く、管理者はオプトインで制御可能な方法でセグメントを共有できます。
...
* [同意および環境設定フィールドグループ](https://experienceleague.adobe.com/docs/experience-platform/xdm/field-groups/profile/consents.html?lang=ja)を使用して、ID（idSpecific）に固有の共有属性を設定する。
  ...
  このフィールドグループは、同意と環境設定の情報を取り込むために、単一のオブジェクトタイプのフィールド「同意」を提供します。[!UICONTROL セグメントの一致 &#x200B;] デフォルトでは、次のように、明示的にオプトアウトされていないすべての ID が含まれます。
...
6. 次に、広告サーバーまたは SSP は、設定されたターゲティング条件を満たす広告インプレッションを配信する。 （フリークエンシーキャップなどの追加のターゲティング条件は、保証された契約と優先される契約のどちらが契約で確立されたかに応じて、広告サーバーやDSPを通じて利用できます）。
