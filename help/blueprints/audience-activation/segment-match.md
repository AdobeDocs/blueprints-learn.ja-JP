...
サードパーティの Cookie とデバイス識別子に関する制限が増えているので、プログラム広告は、オーディエンスを構築してターゲットにする新しい方法を探しています。 「ユニバーサル ID」ソリューションはますます多く提案されていますが、適用可能なプライバシーに関する懸念のバランスを取りながら、同じレベルのターゲティングを達成するための拡張性の高い方法で、業界は依然として不安定です。

Adobe Experience Platform Segment Match をプライバシー中心のオーディエンスコラボレーションで使用して、広告主と発行者間のプログラム的なプライベート契約を強化できます。 セグメント一致を使用すると、次のことが可能になります。

* 分割 **広告トラフィック指定** および **対象ユーザ** ワークフロー。
* パートナーブランドは、相互に共有するオーディエンスメタデータを共有し、同意を得るプロセス内で、ハッシュ化された電子メールやハッシュされた電話番号などの永続的な識別子を使用して ID を同意できます。

### ユースケース

* プログラム的な非公開契約を通じてファーストパーティオーディエンスのターゲティングをおこなう。
* プログラム的なプライベート取引によるファーストパーティオーディエンスの抑制。
* プログラム的なプライベート契約を通じてシードされたファーストパーティオーディエンスからの類似オーディエンスのターゲティング。

>[!BEGINSHADEBOX]

**次のブランド (Luma) とメディアネットワーク (ACME) 間のワークフローの例を考えてみましょう。**

1. ブランド (Luma) は、セグメントマッチを介してメディアネットワーク (ACME) とのオーディエンスマッチを実行します。
2. ACME はAdobe Real-Time CDPの宛先を介してオーディエンスを広告サーバーまたはプログラム SSP にプッシュします。
3. ACME は、前の手順で確立されたオーディエンスを含む、該当するターゲット条件を使用して、プライベート在庫契約 (ID) を設定します。 その後、プライベート在庫取引 ID が Luma のDSPにプッシュされます。
4. Luma は、プライベート在庫契約を設定し、キャンペーン/広告クリエイティブをトラフィックします。
5. キャンペーンは、プログラム的なプライベート在庫取引を通じて提供します。
6. 次に、広告サーバーまたは SSP は、確立されたターゲット条件を満たす広告インプレッションを配信します。 ( 頻度キャップなどの追加のターゲティング条件は、契約で保証契約と優先契約のどちらが成立したかに応じて、広告サーバーやDSPを通じて使用できます )。
7. トラフィックは Luma のブランドプロパティに導かれます。
8. 次に、ACME は、再ターゲティングのために、セグメントマッチを介してキャンペーン後のインサイトまたはオーディエンスを共有します。

>[!ENDSHADEBOX]

![ブランドと発行者の間のワークフローを示す図。](./assets/segment-match-blueprints.png)

>[!IMPORTANT]
>
> 上記のソリューションでは、プログラム的な非公開契約を通じてファーストパーティデータを簡単にターゲット設定できますが、実行前にいくつかの考慮事項がある場合があります（以下の例に限定されません）。
>
>* 同意：この方法でデータを活用するために、ブランド、発行者、または小売メディアネットワークによる適用可能な同意の収集。
>
>* ポリシーとライセンス契約：この方法でデータを活用およびアクティブ化するために、ブランド、発行者、小売メディアネットワークによる適用可能なポリシー（プライバシーポリシー、サードパーティベンダー契約を含む）に準拠します。
