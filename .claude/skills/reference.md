...
| タイプ | 目的 | 場所/例 |
|------|---------|--------------------|
| **概要/ ハブ** | 製品またはエリアを紹介します（シナリオブループリントへのリンク） | e.g. `overview.md`, `journey-optimizer-overview.md` |
| **シナリオのブループリント** | 単一のユースケース：アーキテクチャ、手順、ガードレール | e.g. `real-time-lookup.md`, `journey-optimizer-journeys.md` |
...
| **実装の手順** | パスが定義されたシナリオブループリントの場合 | 番号付きリスト 各ステップ：アクション +手順が存在するExperience Leagueへのリンク。 完全なプロシージャはコピーしないでください。 |
...
| Experience League（en） | `https://experienceleague.adobe.com/en/docs/` | 上記と同じ構造で、`/en/` を使用します。 |
...
- **説明リンクテキスト**:「ここをクリック」しませ `[Create schemas](https://experienceleague.adobe.com/...)`。
